...
public class Agent {
    ...
    public int distance = 0;      // agent distance travelled in one generation 
    ...
    public Agent(int x, int y, int h, int w) {
        ...
        height = h;
        width = w;
        // 1 hidden layer with 10 neurons and 5 inputs (coordinates of finish line (x,y) and 4 outputs,
    ...
    public void perform_action(double[] inputs) {
        ...
        // Math.round(actions[0]) == 1
        if (actions[0] > 0 && x >= 10) {
            x -= 5;
            distance += 5;
        }
        if (actions[1] > 0 && Math.round(inputs[4]) != 1 && x < 1000) {
            x += 5;
            distance += 5;    
        }
        if (actions[2] > 0 && y >= 10) {
