...
public class ObstacleCourse extends JPanel {
    ...
    public ObstacleCourse(int w, int h) {
        ...
        walls = new Wall[] {new Wall(200, win_height-600, 400, 15)};
...
//         }
         for (int i = 1; i < agents.length; i++) {
            agents[i].draw(g, Color.BLUE);
         }
         most_fit.draw(g, Color.GREEN);
    }
    ...
    private class Listener implements ActionListener {
       public void actionPerformed(ActionEvent e)	//this is called for each timer iteration
       {
        ...
        if (most_fit.fitness(goalx, goaly) <= 15 && !end) {
            t.setDelay(15);
        ...
        if (timer > 320 && !end) {
            ...
            // TODO: Must fix prev_most_fit gets destroyed during the mutation process
...
//             most_fit_hidden_layers = most_fit.hl_copy(most_fit.brain.hidden_layers);  // make copy so it is not destroyed during the mutation process
//             most_fit_output_layer = most_fit.deepCopy(most_fit.brain.output_layer);    
            agents[0].brain.hidden_layers = most_fit.hl_copy(most_fit.brain.hidden_layers);
            agents[0].brain.output_layer = most_fit.deepCopy(most_fit.brain.output_layer);
            agents[0].x = startx;
            agents[0].y = starty;
            agents[0].distance = 0;
            // 
            for (int i = 1; i < num_of_agents; i++) {
                agents[i].new_brain(most_fit.hl_copy(most_fit.brain.hidden_layers), most_fit.deepCopy(most_fit.brain.output_layer));
                agents[i].x = startx;
                agents[i].y = starty;
                agents[i].distance = 0;
            }
